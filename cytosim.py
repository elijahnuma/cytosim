...
def searchcytosiminfo(request, mode):
    ...
    if mode == 'test':
        ...
        with open('cytosiminformation.txt', 'r') as f:
            ...
            time_frames_key, test_range_key, sim_time, motor_type = eval(test_info)
        ...
        return (description, time_frames, test_range, sim_time, motor_type, attach_name)
    if mode == 'group':
        group_name = request
        with open('cytosiminformation.txt', 'r') as f:
            file = f.read().splitlines()
...
def metadata(info_num, log=True, show_plot=True):
    ...
    for i, var in enumerate(var_range):
        ...
        if show_plot:
            ...
            plt.savefig(cwd + f"\\cytosimplots\\metadata\\computationaltime\\messages{info_num}{fignamelog}{var_name.replace(' ', '').lower()}{var}.png")
            # memory plot
            fig, ax = plt.subplots(figsize=(plot_length, plot_height))
            ...
            ax.legend()
            ax.grid(True, which='both')
            plt.savefig(cwd + f"\\cytosimplots\\metadata\\memoryusage\\memory{info_num}{fignamelog}{var_name.replace(' ', '').lower()}{var}.png")
...
def plot_handler(df, title, test_num, description, y_label):
    ...
    fig.savefig(cwd + f'\\cytosimplots\\plotsvstime\\{description}\\test{test_num}_{description}.png', bbox_inches='tight')
    
# %% Main loops
for group_sec in [2, 5, 10]:
    ...
    groups = [f'{m} motors ({group_sec} sec)' for m in sorted(set([10**o + j*10**o for o in range(2, 5) for j in range(0, 10)]))]
    for group in groups:  
        ...
        for test_number in group_tests:
            ...
            test_description, time_frames, test_range, sim_time, motor_type, attach_name = searchcytosiminfo(test_number, 'test')
            ...
            for sim in range(sim_num): 
                df_cluster_size_list.append(pd.read_csv(cwd + f'\\tests\\test_{test_number}\\{sim}\\Data_Files\\Rdata.csv', names=time_frames).set_index(test_range).transpose().rename_axis('Second (s)'))
                df_contraction_list.append(pd.read_csv(cwd + f'\\tests\\test_{test_number}\\{sim}\\Data_Files\\Cdata.csv', names=time_frames).set_index(test_range).transpose().rename_axis('Second (s)'))
            ...
            if attach_name != None:
                ...
                plot_handler(df=df_report, title=attach_title, test_num=test_number, description='attachhands', y_label='Hands attached')
            ...
            plot_handler(df=df_cluster, title=cluster_title, test_num=test_number, description='work', y_label='Cluster size')
            # contraction rate over time plot
            contraction_title = f'{test_description} {title_parenthetical} (Average over {sim_num} sims)'
            plot_handler(df=df_contraction, title=contraction_title, test_num=test_number, description='power', y_label='Contraction rate')
            # cluster size delta from beginning to end plot
            ...
            df_max_contraction.plot(kind='line', figsize=(plot_length, plot_height), color=color, linestyle=linestyle, ax=ax_max_contraction, title=f'Max contraction rate {title_parenthetical}', logx=group_log).set(ylabel='Max contraction rate (um/s)')        
            # max contraction rate plot
            df_max_contraction_time.plot(kind='line', figsize=(plot_length, plot_height), color=color, linestyle=linestyle, ax=ax_max_contraction_time, title=f'Max contraction rate time {title_parenthetical}', logx=group_log).set(ylabel='Max contraction rate time (s)')  
        # save delta, doesn't save if individual test done
        if group != 'manual':
            fig_cluster_delta.savefig(cwd + f"\\cytosimplots\\plotsvsbindingrange\\work\\work{group.replace(' ', '')}.png", bbox_inches='tight')
            fig_max_contraction.savefig(cwd + f"\\cytosimplots\\plotsvsbindingrange\\maxpower\\maxpower{group.replace(' ', '')}.png", bbox_inches='tight')
            fig_max_contraction_time.savefig(cwd + f"\\cytosimplots\\plotsvsbindingrange\\maxpowertime\\maxpowertime{group.replace(' ', '')}.png", bbox_inches='tight')
            fig_attach_delta.savefig(cwd + f"\\cytosimplots\\plotsvsbindingrange\\attachdelta\\attachdelta{group.replace(' ', '')}.png", bbox_inches='tight')
    ...
    # % Initializer
    motor_list, heads_list, binding_ranges, *_ = searchcytosiminfo(metadata_num, 'metadata')
    ...
    for motor in motor_list:
        ...
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsbindingrange\\motorheadsuperposition\\clustersizedeltasmotors{sim_time}sec{vs_title}.png")
        plt.close()
    # compared heads
    for head in heads_list:
        ...
        pd.concat(dfs, axis=1).plot(kind='line', figsize=(plot_length, plot_height), style=styles, title='Cluster size delta', logx=True).set(ylabel='Cluster size delta difference (um)')
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsbindingrange\\motorheadsuperposition\\clustersizedeltasheads{sim_time}sec{vs_title}.png")
        plt.close()
    ...
    for binding_range in binding_ranges:
        ...
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsmotors\\efficiency\\work\\worktimeefficiency{sim_time}sec{binding_range}bindingrange.png")
        ...
        plt.grid(True, which='both')
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsmotors\\efficiency\\work\\workmemoryefficiency{sim_time}sec{binding_range}bindingrange.png")
        ...
        plt.grid(True, which='both')
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsmotors\\work\\work{sim_time}sec{binding_range}bindingrange.png")
        ...
        plt.grid(True, which='both')
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsmotors\\efficiency\\power\\powertimeefficiency{sim_time}sec{binding_range}bindingrange.png")
        ...
        plt.grid(True, which='both')
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsmotors\\efficiency\\power\\powermemoryefficiency{sim_time}sec{binding_range}bindingrange.png")
        ...
        plt.grid(True, which='both')
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsmotors\\maxpower\\maxpower{sim_time}sec{binding_range}bindingrange.png")
        ...
        plt.grid(True, which='both')
        plt.savefig(cwd + f"\\cytosimplots\\plotsvsmotors\\maxpowertime\\maxpowertime{sim_time}sec{binding_range}bindingrange.png")
# %% Tracking diffusion
# intializers
test_number = 579
...
plt.savefig(os.getcwd() + f"\\cytosimplots\\\diffusion\\{motor_type}diffusion.png")
# %% Compiler data
starting_job = 410
time_frames = 3
test_range = 1
...
for t, tup in enumerate([(i, k) for i in motor_list for k in heads_list], starting_job):
    m, h = tup
    print(f"{t}: 'Rod {h} heads' ({time_frames}, {test_range}, {sim_time}, 'rod') # {m} motors") if h == heads_list[0] else print(f"{t}: 'Rod {h} heads' (3, 1, {sim_time}, 'rod')")
# %% Groups in cytosiminformation.txt 
for i, m in enumerate(motor_list):
    heads_num = len(heads_list)
    print(f"{m} motors ({sim_time} sec): ({[starting_job + heads_num*i + j for j in range(heads_num)]}, True, '({m} motors)', {metadata_num})")
# %% Metadata generator
